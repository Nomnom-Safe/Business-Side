...
const MenuItemsPage = () => {
	...
	const handleSave = async (updatedItem) => {
		try {
			const result = await api.menuItems.updateItem(
				updatedItem.id,
				updatedItem,
			);
			if (!result.ok) {
	...
	const filteredItems = useMemo(() => {
		return menuItems.filter((item) => {
			...
			const matchesCategory =
				activeCategory === 'all' || itemTypes.includes(activeCategory);
	...
	const sortedItems = useMemo(() => {
		...
		list.sort((a, b) => {
			...
			if (sortBy === 'name_desc')
				return (
					nameFor(b).localeCompare(nameFor(a)) ||
	...
	const hasRealMenu =
		fetchedMenu?.id && !String(fetchedMenu.id).startsWith('menu_');
	...
	const saveMenuTitle = async () => {
		...
		try {
			const result = await api.menus.update(fetchedMenu.id, {
				title: newTitle,
			});
			if (result.ok && result.data) {
				setFetchedMenu((prev) =>
					prev ? { ...prev, title: result.data.title ?? newTitle } : prev,
				);
			...
			<div className='menu-items-page__container'>
				...
				<div className='menu-items-page__header'>
					...
					<div className='menu-items-page__title-wrap'>
						{isEditingTitle ? (
							...
							>
								<h1 className='menu-items-page__title'>{displayTitle}</h1>
								{hasRealMenu && (
									<button
										type='button'
				...
				<div className='menu-items-page__filter-bar'>
					<div className='menu-items-page__search-wrapper'>
						<div
							className='menu-items-page__search-prefix'
							aria-hidden='true'
						>
				...
				{showFilters && (
					<div className='menu-items-page__filters-panel'>
						<div className='menu-items-page__filter-group'>
							<label className='menu-items-page__filter-label'>
								Availability
							</label>
						...
						{hasActiveFilters && (
							<button
								className='menu-items-page__clear-filters'
						...
						<>
							<div className='menu-items-page__list-header'>
								...
								<div className='menu-items-page__sort'>
									<label
										htmlFor='sort-by'
										className='menu-items-page__sort-label'
									>
										Sort by
									</label>
									<select
