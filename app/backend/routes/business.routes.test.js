...
describe('Business Routes', () => {
	...
	test('GET /api/businesses - returns all businesses', async () => {
		...
		const res = await request(app).get('/api/businesses').expect(200);
	...
	test('DELETE /api/businesses/:id - deletes a business', async () => {
		...
		await request(app).delete(`/api/businesses/${biz._id}`).expect(200);
	...
	test('GET /api/businesses/:id - returns 404 if not found', async () => {
		...
		await request(app).get(`/api/businesses/${fakeId}`).expect(404);
	});
});
